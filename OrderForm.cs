...
namespace Inventory_Management_System
{
    public partial class OrderForm : Form
    {
        ...
        public void LoadOrder()
        {
            ...
            {
                i++;
                ddgOrder.Rows.Add(i, dr[0].ToString(),Convert.ToDateTime(dr[1].ToString()).ToString("dd/MM/yyyy"),dr[2].ToString(),dr[3].ToString(),dr[4].ToString(),dr[5].ToString(),dr[6].ToString(),dr[7].ToString(),dr[8].ToString());
        ...
        {
            string colName = ddgOrder.Columns[e.ColumnIndex].Name;            
            if (colName == "Delete")
            {
                ...
                {
                    con.Open();
                    cm = new SqlCommand("DELETE FROM tbOrder WHERE orderid LIKE '" + ddgOrder.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", con);
                    cm.ExecuteNonQuery();
                    con.Close();
                    MessageBox.Show("Order has been successfully deleted");

                    cm = new SqlCommand("UPDATE tbProduct SET pqty=(pqty+@pqty) WHERE pid LIKE '" + ddgOrder.Rows[e.RowIndex].Cells[3].Value.ToString() + "'", con);
                    cm.Parameters.AddWithValue("@pqty", Convert.ToInt16(ddgOrder.Rows[e.RowIndex].Cells[3].Value.ToString()));
                    con.Open();
                    cm.ExecuteNonQuery();
                    con.Close();
                }
