...
namespace Inventory_Management_System
{
    public partial class CategoryForm : Form
    {
        ...
        public void LoadCategory()
        {
            ...
            {
                i++;
                ddgCategory.Rows.Add(i, dr[0].ToString(), dr[1].ToString());
            }
            dr.Close();
            con.Close();
        }
        ...
        {
            string colName = ddgCategory.Columns[e.ColumnIndex].Name;
            if (colName == "Edit")
            {
                CategoryModuleForm formModule = new CategoryModuleForm();
                formModule.lblCatid.Text = ddgCategory.Rows[e.RowIndex].Cells[1].Value.ToString();
                formModule.txtcatname.Text = ddgCategory.Rows[e.RowIndex].Cells[2].Value.ToString();
            ...
            else if (colName == "Delete")
            {
                ...
                {
                    con.Open();
                    cm = new SqlCommand("DELETE FROM tbCategory WHERE catid LIKE '" + ddgCategory.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", con);
                    cm.ExecuteNonQuery();
                    con.Close();
